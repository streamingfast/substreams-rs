...
- **Breaking**: protobuf encoding and decoding moved from [prost](https://github.com/tokio-rs/prost) to
  ...
  Handlers can now opt into buffa's lazy views, which decode nested and repeated message fields only
  when they are read. A module that touches a few fields of a large block no longer pays for the rest:

  ```rust
  #[substreams::handlers::map(lazy)]
  ...
  Without the option, handlers keep taking an owned message.
