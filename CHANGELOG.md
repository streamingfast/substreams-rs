...
- **Breaking**: protobuf encoding and decoding moved from [prost](https://github.com/tokio-rs/prost) to
  [buffa](https://github.com/anthropics/buffa). `prost` is no longer a dependency.

  Handlers can now use buffa's lazy views, which decode nested and repeated message fields only
  when they are read. A module that touches a few fields of a large block no longer pays for the
  rest. A handler opts in through its own signature, by declaring the view rather than an owned
  message:
  ...
  Types generated by buffa differ from prost in three ways that affect handler code:

  | | prost | buffa |
  |---|---|---|
  | proto3 enum field | `i32` | `EnumValue<E>` — `.to_i32()`, `.as_known()`, `E::Variant.into()` |
  | singular message field | `Option<Box<T>>` | `MessageField<T>` — derefs to a default instance, `.as_option()` for interop |
  | encoding | `Result<Vec<u8>, EncodeError>` | infallible `Vec<u8>` |

  Signature changes on this crate:
  ...
  - `proto::decode`, `proto::decode_ptr`, `output`, `StoreSetProto`, `StoreSetIfNotExistsProto`,
    `StoreGetProto` and `DeltaProto` are bound on `buffa::Message` rather than `prost::Message`.
  - `StoreDelta::operation` is an `EnumValue<Operation>`; `Clock::timestamp` is a `MessageField`.
  - `StoreDeltas::deltas` is now `store_deltas`, matching the schema. The wire format is unchanged;
    the previous name came from generated code that had fallen behind the `.proto`.

- **Breaking**: `pb` types are generated at build time from the `.proto` files under `substreams/proto`
  rather than checked in, so they cannot drift from the schema again. The service-plane messages
  (`Request`, `Response`, `BlockScopedData`, `ModuleOutput`, `ModuleProgress`, `ModulesProgress`,
  `BlockRange`, `InitialSnapshotData`, `InitialSnapshotComplete`, `Output`, `ForkStep`, `Modules`,
  `Module`, `Binary`) are no longer generated: they are the client/server protocol and are unused by
  WASM modules.

### Removed

- **Breaking**: the experimental quick-protobuf support added in this release cycle
  (the `quick-protobuf` feature, the `quick_protobuf` handler option and the `substreams::quick`
  module). buffa supersedes it.

- `substreams::buffa::output`, which duplicated `substreams::output` once both took a
  `buffa::Message`.

### Fixed

- `cargo test` works again. `.cargo/config.toml` set `wasm32-unknown-unknown` as the default target
  for every cargo command, so the test binary was built as WASM and then run as a native executable.
  The target is now named explicitly where it is needed.
